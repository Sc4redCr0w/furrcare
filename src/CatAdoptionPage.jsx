...
const CatAdoptionPage = ({ onGoHome, onGoBack, user, onLogout, onNavigateToCheckup, onNavigateToGrooming }) => {
  ...
  useEffect(() => {
    const fetchCats = async () => {
      try {
        ...
        const imagePromises = data.map((cat) => {
        ...
        console.error('Error fetching cats data:', e);
  ...
  const handleCatClick = (cat) => setSelectedCat(cat);
  const closeCatDetails = () => setSelectedCat(null);
        ...
        {selectedCat && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 bg-black/80 backdrop-blur-sm flex items-center justify-center p-4 z-50">
            <motion.div initial={{ scale: 0.8, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.8, opacity: 0 }} className="bg-gray-900 rounded-2xl p-8 max-w-2xl w-full max-h-[90vh] overflow-y-auto">
              <div className="flex justify-end mb-4"><button onClick={closeCatDetails} className="text-white/70 hover:text-white text-2xl w-8 h-8 flex items-center justify-center rounded-full hover:bg-white/10 transition-colors duration-300">×</button></div>
              <div className="flex flex-col lg:flex-row gap-8">
                ...
                <div className="lg:w-1/2 space-y-6">
                  ...
                  <div className="space-y-3">
                    ...
                    <button onClick={closeCatDetails} className="w-full bg-white/10 hover:bg-white/20 text-white px-6 py-3 rounded-lg font-medium">Close</button>
                  </div>
                </div>
              </div>
            </motion.div>
