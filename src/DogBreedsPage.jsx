...
const DogBreedsPage = ({ onGoHome, onAdoptNow, user, onLogout, onNavigateToCheckup, onNavigateToGrooming }) => {
  ...
  const handlePrev = () => {
    setCurrentIndex((prev) => (prev === 0 ? dogBreeds.length - 1 : prev - 1));
  };

  const handleNext = () => {
    setCurrentIndex((prev) => (prev === dogBreeds.length - 1 ? 0 : prev + 1));
  };
