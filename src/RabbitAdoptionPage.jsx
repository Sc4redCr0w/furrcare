            ...
            {filteredRabbits.map((pet, index) => (
              <motion.div key={pet.id} initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} transition={{ delay: 0.1 * index }} className="text-white rounded-2xl p-6 hover:shadow-2xl hover:shadow-white/20 transition-all duration-300 hover:-translate-y-2 cursor-pointer" style={{ backgroundColor: 'rgb(13, 17, 23)' }} onClick={()=>setSelectedRabbit(pet)}>
                ...
                <button 
                  ...
                  onClick={(e) => { e.stopPropagation(); setSelectedRabbit(pet); }}
                >
                  View Details 👁️
                </button>
              </motion.div>
        ...
        {selectedRabbit && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 bg-black/80 backdrop-blur-sm flex items-center justify-center p-4 z-50">
            <motion.div initial={{ scale: 0.8, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.8, opacity: 0 }} className="bg-gray-900 rounded-2xl p-8 max-w-2xl w-full max-h-[90vh] overflow-y-auto">
              <div className="flex justify-end mb-4"><button onClick={()=>setSelectedRabbit(null)} className="text-white/70 hover:text-white text-2xl w-8 h-8 flex items-center justify-center rounded-full hover:bg-white/10 transition-colors duration-300">×</button></div>
              <div className="flex flex-col lg:flex-row gap-8">
                ...
                <div className="lg:w-1/2 space-y-6">
                  ...
                  <div className="space-y-3">
                    ...
                    <button onClick={()=>setSelectedRabbit(null)} className="w-full bg-white/10 hover:bg-white/20 text-white px-6 py-3 rounded-lg font-medium">Close</button>
                  </div>
                </div>
              </div>
            </motion.div>
