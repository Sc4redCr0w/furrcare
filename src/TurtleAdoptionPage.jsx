...
const TurtleAdoptionPage = ({ onGoHome, onGoBack }) => {
  ...
  useEffect(() => {
    const fetchTurtles = async () => {
      try {
        const res = await fetch('/turtles.json');
        if (!res.ok) throw new Error('Failed to load turtles.json');
    ...
    <div className="min-h-screen w-full bg-black text-white relative overflow-hidden">
      <header className="flex items-center justify-between p-6 border-b border-white/20 relative z-10">
        ...
        <div className="flex items-center gap-4">
          <button onClick={onGoBack} className="text-white hover:text-orange-400 transition-colors duration-300 underline">Back to Turtle Species</button>
      ...
      <div className="px-8 py-6 relative z-10">
        <div className="max-w-7xl mx-auto">
          <motion.div initial={{ y: -20, opacity: 0 }} animate={{ y: 0, opacity: 1 }} className="text-center mb-8">
            <h1 className="text-4xl lg:text-5xl font-bold mb-4"><span className="text-white">ADOPT A </span><span className="text-orange-400">TURTLE</span></h1>
            <p className="text-white/80 text-lg">Find a calm shelled friend</p>
          </motion.div>
          ...
          <motion.div initial={{ y: 20, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ delay: 0.2 }} className="bg-white/5 backdrop-blur-sm rounded-2xl p-6 mb-8">
            <div className="flex flex-wrap gap-4 items-center">
              ...
              <Dropdown value={filters.breed} onChange={(v)=>handleFilterChange('breed', v)} placeholder="All Species" options={[
                {value:'',label:'All Species'},{value:'Red-eared slider',label:'Red-eared slider'},{value:'Painted turtle',label:'Painted turtle'},{value:'Box turtle',label:'Box turtle'}]}/>
            ...
            <div className="mt-4 text-white/70">Showing {filteredTurtles.length} of {turtles.length} turtles</div>
          </motion.div>
          ...
          {filteredTurtles.length===0 && turtles.length>0 && (
            <div className="text-center py-12">
              <p className="text-white/70 text-lg">No turtles found matching your criteria.</p>
            ...
            <motion.div initial={{ scale: 0.8, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.8, opacity: 0 }} className="bg-gray-900 rounded-2xl p-8 max-w-2xl w-full max-h-[90vh] overflow-y-auto">
              ...
              <div className="flex flex-col lg:flex-row gap-8">
                ...
                <div className="lg:w-1/2 space-y-6">
                  <h2 className="text-4xl font-bold text-orange-400">{selectedTurtle.name}</h2>
                  <p className="text-white/80">{selectedTurtle.breed} • {selectedTurtle.age} yrs • {selectedTurtle.location}</p>
                  <p className={selectedTurtle.vaccination==='yes'?'text-green-400 font-medium':'text-red-400 font-medium'}>{selectedTurtle.vaccination==='yes'?'✓ Vaccinated':'✗ Not Vaccinated'}</p>
...
export default TurtleAdoptionPage;
